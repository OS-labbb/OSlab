...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
  ...
  //LAB8:EXERCISE1 2312711 2314003 2314081 HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	/*
	 ...
	 *       NOTICE: useful function: sfs_bmap_load_nolock, sfs_buf_op	
	*/
    // (1) 如果 offset 没有与块对齐，先处理第一个不完整的块
    ...
    // (3) 如果结束位置没有与块对齐，处理最后一个不完整的块
  
    size = endpos % SFS_BLKSIZE;
    if (size != 0 && alen < endpos - offset) {
        if ((ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino)) != 0) {
            goto out;
        }
