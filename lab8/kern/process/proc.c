...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
        ...
        //LAB8 2312711 2314003 2314081 : (update LAB6 steps)
        /*
         * below fields(add in LAB6) in proc_struct need to be initialized
        ...
        proc->filesp = NULL;  // LAB8: 初始化文件结构指针
...
void proc_run(struct proc_struct *proc)
{
    // LAB4:填写你在lab4中实现的代码
        ...
        */
    //LAB8 2312711 2314003 2314081 : (update LAB4 steps)
      /*
       * below fields(add in LAB6) in proc_struct need to be initialized
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    ret = -E_NO_MEM;
    // LAB8:EXERCISE2 2312711 2314003 2314081 HINT:how to copy the fs in parent's proc_struct?
    // LAB4:填写你在lab4中实现的代码
    /*
     * Some Useful MACROs, Functions and DEFINEs, you can use them in below implementation.
     * MACROs or Functions:
...
{
    /* LAB8:EXERCISE2 2312711 2314003 2314081 HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
    ...
    if (pgdir_alloc_page(mm->pgdir, USTACKTOP - PGSIZE, perm) == NULL) {
        goto bad_cleanup_mmap;
    }
    if (pgdir_alloc_page(mm->pgdir, USTACKTOP - 2 * PGSIZE, perm) == NULL) {
        goto bad_cleanup_mmap;
    }
    if (pgdir_alloc_page(mm->pgdir, USTACKTOP - 3 * PGSIZE, perm) == NULL) {
        goto bad_cleanup_mmap;
    }
    if (pgdir_alloc_page(mm->pgdir, USTACKTOP - 4 * PGSIZE, perm) == NULL) {
        goto bad_cleanup_mmap;
    }
    ...
    // 计算字符串区起始地址（向下对齐到16字节）
    uintptr_t stacktop = USTACKTOP - argv_size;
    stacktop &= ~0xF;  // 16字节对齐
    
    // argv 指针数组的位置
    uintptr_t uargv_base = stacktop - argc * sizeof(char *);
    uargv_base &= ~0xF;  // 16字节对齐
    
    // 检查是否超出栈空间
    if (uargv_base < USTACKTOP - USTACKSIZE) {
        ret = -E_NO_MEM;
        goto bad_cleanup_mmap;
    }
    
    // 通过物理地址复制参数
    uintptr_t str_addr = stacktop;
    for (i = 0; i < argc; i++) {
        ...
        for (size_t j = 0; j < len; j++) {
            // 获取目标虚拟地址对应的物理页
            pte_t *ptep = get_pte(mm->pgdir, str_addr + j, 0);
            ...
            char *dst = (char *)page2kva(p) + ((str_addr + j) & (PGSIZE - 1));
            *dst = kargv[i][j];
        }
        
        // 设置 argv[i] 指针
        uintptr_t argv_ptr_addr = uargv_base + i * sizeof(char *);
        pte_t *ptep = get_pte(mm->pgdir, argv_ptr_addr, 0);
        if (ptep == NULL || !(*ptep & PTE_V)) {
            ret = -E_INVAL;
            goto bad_cleanup_mmap;
        }
        ...
        struct Page *p = pte2page(*ptep);
        char **dst = (char **)((char *)page2kva(p) + (argv_ptr_addr & (PGSIZE - 1)));
        *dst = (char *)str_addr;
    ...
    // 设置 argc 和 argv
    tf->gpr.a0 = argc;
    tf->gpr.a1 = uargv_base;  // argv 指针数组的地址
    
    // 设置 status：清除 SPP（用户模式），设置 SPIE（使能中断）
