...
/*
 * copy_range - Copy or share memory pages between processes
 * 
 * @to:    destination page directory
 * @from:  source page directory  
 * @start: start virtual address
 * @end:   end virtual address
 * @share: if true, use COW; if false, do traditional copy
 * 
 * Modified to support COW:
 * - When share=true, pages are shared with COW flag instead of being copied
 * - When share=false, traditional copy is performed
 */
int copy_range(pde_t *to, pde_t *from, uintptr_t start, uintptr_t end, bool share)
{
    ...
    // COW optimization: if share is true, use COW mechanism
    if (share) {
        cprintf("Using COW mechanism for range [0x%x, 0x%x)\n", start, end);
        ...
        do {
            ...
            if (*ptep & PTE_V) {
                ...
                if (!already_cow && (perm & PTE_W)) {
                    // First time sharing this writable page
                    ...
                    *ptep = pte_create(page2ppn(page), new_perm);
                    ...
                    cprintf("COW: Marked parent page at 0x%x as COW\n", start);
                }
                
                // Set up child's PTE with COW flag (no write permission)
                uint32_t child_perm = (perm & ~PTE_W) | PTE_COW;
                
                // Share the page by incrementing reference count
                page_ref_inc(page);
                
                // Map the shared page to child
                *nptep = pte_create(page2ppn(page), child_perm);
                
                cprintf("COW: Shared page at 0x%x with child (ref=%d)\n", 
                        start, page_ref(page));
            }
    ...
    // Traditional copy (original implementation)
    cprintf("Using traditional copy for range [0x%x, 0x%x)\n", start, end);
