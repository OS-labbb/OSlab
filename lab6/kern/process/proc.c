...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
        ...
        // LAB6:2312711 2314003 2314081 (update LAB5 steps)
        /*
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    // 步骤3: 根据clone_flags调用copy_mm复制或共享内存空间
...
load_icode(unsigned char *binary, size_t size)
{
    ...
    // 设置程序计数器 - 指向ELF入口点
    //这个入口地址指向用户程序的main函数或其他指定的起始执行点。
    tf->epc = elf->e_entry;

    // 设置状态寄存器 - 为用户模式配置
    // SPP: 前一个特权级模式 (0=User, 1=Supervisor)
    // SPIE: 前一个中断使能状态
    // 我们需要设置状态为用户模式，并启用中断
    tf->status = (sstatus & ~SSTATUS_SPP) |  // 清除SPP位，设置为用户模式
                 SSTATUS_SPIE;               // 设置SPIE，在用户模式下启用中断
