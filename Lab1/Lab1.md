...
**实际执行** ：在entry.S中，我们已经为栈分配好了内存，但此时CPU并不知道栈在哪里，通过执行这条语句，我们让sp寄存器指向了栈顶，现在CPU知道从哪里开始用栈了。

##### 目的和意义

- 为C代码运行准备栈环境，如果没有栈，C函数无法运行，因为sp指向的内存可能不存在或不可写
...
- 尾调用优化避免了不必要的栈帧创建，复用当前栈帧，避免资源浪费
- 内核初始化不会返回，所以不需要保存返回地址
- 体现操作系统的层次化设计
