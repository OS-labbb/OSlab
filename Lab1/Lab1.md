...
在分析具体指令前，首先理解完整的内核启动流程：

> BIOS自检 → 加载引导扇区(0x7C00) → 引导扇区设置保护模式 → 加载内核到内存(0x100000) → 跳转到kern_entry → 执行kern_init

#### 2.`la sp,bootstacktop`指令分析
...
由上述分析我们可以发现，我们的代码会在地址为` 0x101``0`` `地址的指令处发生跳转，故实际执行的指令仅为前五条指令。这些指令完成了最基本的硬件初始化操作，包括获取硬件线程ID和为跳转到下一引导阶段做准备。
...
- New value的十六进制值 = **0x80000470** ：将New value转成十六进制，这是新的异常向量表的地址
...
```Plain
# 查看异常向量表的内容
(gdb) x/20i 0x800003f0
```

![](pic/14.png)

```Plain
0x800003f0: jalr    a6,263(t1)     # 跳转并链接，保存返回地址
0x800003f4: vand.vi v0,v31,6       # 向量指令（可能是填充）
0x800003f8: fld     fa2,160(s0)    # 浮点加载指令
0x800003fa: bne     a4,a5,0x800003f6  # 条件分支
0x800003fe: csrr    a0,mscratch    # 读取 mscratch 寄存器
0x80000402: jal     ra,0x800005e2  # 调用异常处理函数
0x80000406: j       0x80000468     # 无条件跳转
```

通过观察此地址后面的7条指令再次印证，异常向量表地址发生了改变。监控 mtvec 寄存器的变化显示，异常向量表地址从初始的 0 更新为 0x80000470。

我们查看新的向量表位置附近的相关指令，分析此处执行了什么内容：

```Plain
# 查看新的异常向量表
(gdb) x/20i 0x80000470
```

![](pic/15.png)

我们发现这是一个完整的异常上下文保存程序：

**（1） 上下文切换准备** 

```Plain
csrrw tp,mscratch,tp    # 交换 tp 和 mscratch，获取异常栈指针
sd    t0,64(tp)         # 保存 t0 寄存器到异常栈
```
...
srli  t0,t0,0xb         # 右移11位，提取MPP字段
andi  t0,t0,3           # 保留最低2位（特权级别）
xori  t0,t0,3           # 异或3，检查是否为机器模式
beqz  t0,0x80000496     # 如果是机器模式，跳转
```
...
```Plain
# 如果不是机器模式：
add   t0,sp,zero        # 保存当前栈指针
addi  sp,tp,-272        # 使用异常栈（tp-272）

# 如果是机器模式：
add   t0,sp,zero        # 保存当前栈指针  
addi  sp,sp,-272        # 在当前栈上分配空间
```
...
```Plain
sd    t0,16(sp)         # 保存原栈指针
ld    t0,64(tp)         # 恢复之前保存的t0
sd    t0,40(sp)         # 保存t0到栈
csrrw tp,mscratch,tp    # 恢复原tp值
csrr  t0,mepc           # 读取异常返回地址
sd    t0,256(sp)        # 保存异常返回地址
csrr  t0,mstatus        # 读取机器状态
sd    t0,264(sp)        # 保存机器状态
```

上面这种设计确保了异常处理过程对正常程序执行的透明性，即异常处理完成后能够完全恢复到异常发生前的状态继续执行，这是系统稳定运行的基本保障。
...
```Plain
=> 0x000000008020000a <+0>:    auipc    a0, 0x3      # 计算 edata 地址
   0x000000008020000e <+4>:    addi     a0, a0, -2   
   0x0000000080200012 <+8>:    auipc    a2, 0x3      # 计算 end 地址  
...
kern_init 函数的执行流程体现了操作系统内核启动的标准步骤。首先通过地址计算**确定 BSS 段的范围**，这是 C 语言运行时环境建立的重要步骤，确保所有未初始化的全局变量和静态变量都被正确清零。随后调用 memset 函数**完成 BSS 段的清零操作**，为后续的内核代码执行提供了干净的内存环境。
...
1. **基础环境****搭建**：通过`au``i``pc`指令计算相关地址，为后续操作构建基础地址环境
...
4. **内核初始化****阶段**：执行`kern_init`函数，完成BSS段清零、信息输出设置等工作，最终进入空闲循环。
